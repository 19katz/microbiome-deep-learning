...
def write_command(batch, kmer_size):
    name='Karlsson_2013'
    outdir='Karlsson_2013_no_adapter'
    command="while read file; do\n"
    command += "echo $file\n"
    command += "dir=/pollard/home/ngarud/shattuck/metagenomic_fastq_files/%s/fastq_files_no_adapter/\n" %name
    command += "jellyfish count <(zcat ${dir}/${file}-trimmed-pair1.fastq.gz) <(zcat ${dir}/${file}-trimmed-pair2.fastq.gz) /dev/fd/0 -m %s -s 100M -t 2 -C -F 2 -o ~/deep_learning_microbiome/data/%smers_jf/%s/${file}_%smer.jf \n" %(kmer_size,kmer_size, outdir, kmer_size) 
    command += "jellyfish dump ~/deep_learning_microbiome/data/%smers_jf/%s/${file}_%smer.jf | grep '>' | gzip > ~/deep_learning_microbiome/data/%smers_jf/%s/${file}_%smer.gz\n" %(kmer_size,outdir, kmer_size, kmer_size, outdir, kmer_size)
