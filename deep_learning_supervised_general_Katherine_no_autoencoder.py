...
# as well as dropout
def add_layer_common(model, activation):
    ...
    for dataset_name in exp_configs[dataset_key][0]:
        ...
        for i in range(exp_config[num_iters_key]):
            ...
            for config in config_iter_fold_results:
                ...
                config_results[config].append([val_acc, acc, val_loss, loss, conf_mat, fold_results[:, 3], fold_results[:, 5], fold_results[:, 6]])
...
def loop_over_norm_funcs():
    ...
    *training* samples. This is the key to the predictive/generalization power of the models:
...
def loop_over_layers():
    for layers in exp_configs[layers_key][0]:
        ...
        # The last entry of the layers is for code layer dimension
...
# Counts the number of layers from input layer to the middle code layer
...
def build_train_test(layers):
    """
    This uses the current active exp config (set up by the loops above) to build and train/test the supervised model
    """
    ...
    for i in range(1, len(layers)-1):
        model.add(Dense(layers[i], input_dim=layers[i-1], kernel_regularizer=regularizers.l2(exp_config[act_reg_key]),
                              activity_regularizer=regularizers.l1(exp_config[act_reg_key])))
        add_layer_common(model, exp_config[enc_act_key])

    # the middle code layer - it has an activation function independent of other encoding layers
    i = len(layers) - 1
    model.add(Dense(layers[i], input_dim=layers[i-1], kernel_regularizer=regularizers.l2(exp_config[act_reg_key]),
                          activity_regularizer=regularizers.l1(exp_config[act_reg_key])))
    model.add(Activation(exp_config[enc_act_key if exp_config[code_act_key] == SAME_AS_ENC else code_act_key]))

    # the model from input to the code layer - used for plotting 2D graphs
    code_layer_model = Model(inputs=model.layers[0].input,
                             outputs=model.layers[cnt_to_coding_layer() - 1].output)
    ...
    # categorical cross entropy - not what's in exp_configs
    model.add(Dense(n_classes, input_dim=layers[-1]))
    ...
    super_val_acc = hist['val_acc'][-1]
    super_acc = hist['acc'][-1]
    super_val_loss = hist['val_loss'][-1]
    super_loss = hist['loss'][-1]
    ...
    hist['val_acc'][min_li] + hist['acc'][min_li] + super_val_acc + super_acc
    for i in [ 1, 'macro' ]:
        avg_perf += roc_auc[i]
    avg_perf /= 12

    print(('{}\t{},{},{},{},{}\t{},{},{},{},{}\t{},{},{},{},{}\t{},{},{},{},{}\t{}\t{}\t{}\t{}\t{}\t{}\tsuper-min-loss-max-acc for ' + class_name + ':{}').
          format(avg_perf, hist['val_acc'][max_vai], max_vai, hist['acc'][max_vai], hist['val_loss'][max_vai], hist['loss'][max_vai],
                 ...
                 super_val_acc, super_acc, super_val_loss, super_loss,
                 roc_auc[1], roc_auc['macro'], config_info()))
    ...
    config_iter_fold_results[config_key][iter_ind][fold_ind] = [hist, [codes_pred, info_test], conf_mat, [fpr, tpr, roc_auc], classes, [y_test, y_test_pred],
                                                                [accuracy, f1, precision, recall, roc_auc[1], roc_auc['macro']]]

        
# Plot the 2D codes in the layer right before the final classification layer
def plot_2d_codes(codes, name='super_general_two_codes', title='2D Codes Before Classfication Layer', 
...
# plot loss vs epochs
def plot_loss_vs_epochs(loss, val_loss, name='super_general_loss_vs_epochs', title='Loss vs Epochs', 
...
# plot accuracy vs epochs
def plot_acc_vs_epochs(acc, val_acc, name='super_general_accu_vs_epochs', title='Accuracy vs Epochs',
...
def plot_confusion_matrix(cm, config=None, cmap=pylab.cm.Reds):
    ...
    add_figtexts_and_save(fig, 'super_general_confusion_mat', "Confusion matrix for predicting sample's " + class_name + " status using 5mers", y_off=1.3, config=config)

def plot_roc_aucs(fpr, tpr, roc_auc, accs, std_down=None, std_up=None, config=None, name='super_general_roc_auc', title='ROC Curves with AUCs/ACCs', 
...
config_keys = [ dataset_key, layers_key, enc_dim_key, enc_act_key, code_act_key, 
                super_epochs_key, norm_sample_key, batch_size_key, batch_norm_key,
                # Thes two are not used yet, so skip them to save file name length
                # early_stop_key, patience_key,
                dropout_key, act_reg_key, backend_key, version_key, no_random_key,
                # kfold_key should be the last and iter_key second to last
                use_kfold_key, num_iters_key, shuffle_labels_key, iter_key, kfold_key ]
...
def setup_exp_configs_from_config_info(config):
    ...
    for f in fields:
        ...
        exp_configs[k][0] = v if cnt <= 1 else [v]
...
def add_figtexts_and_save(fig, name, desc, x_off=0.02, y_off=0.56, step=0.04, config=None):
    if add_fig_desc:
        ...
        y_off -= step
        pylab.figtext(x_off, y_off, get_config_desc(shuffle_labels_key))
        
        y_off -= step
        pylab.figtext(x_off, y_off, get_config_desc(layers_key) + ' --- ' + get_config_desc(enc_dim_key))

        y_off -= step
        pylab.figtext(x_off, y_off, get_config_desc(enc_act_key) + ' --- ' + get_config_desc(code_act_key))

        y_off -= step
        pylab.figtext(x_off, y_off, get_config_desc(use_kfold_key) + ' --- ' + get_config_desc(kfold_key))

        y_off -= step
        pylab.figtext(x_off, y_off, get_config_desc(batch_size_key) + ' --- ' + get_config_desc(shuffle_abunds_key))

        y_off -= step
        pylab.figtext(x_off, y_off, get_config_desc(super_epochs_key))

        y_off -= step
        pylab.figtext(x_off, y_off, get_config_desc(batch_norm_key))
...
if __name__ == '__main__':
    # the experiment mode can be one of SUPER_MODELS, SEARCH_SUPER_MODELS, OTHER
    # - see below
    ...
    if exp_mode == "SUPER_MODELS":
        ...
        set_config(dataset_key, ['AllContinent'])
        ...
        dataset_config['AllContinent'] = ['LS:1024-2_ED:2_EA:linear_CA:tanh_SEP:200_NO:L1_BS:16_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1',
                                          'LS:1024-2_ED:2_EA:linear_CA:tanh_SEP:200_NO:L1_BS:16_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1_SL:1'
        ]
        ...
        # the first model is the real one and the second is identical except with labels shuffled (for testing AUC statistical significance as in Pasolli)
        dataset_config['AllCountry'] = ['LS:1024-2-2_ED:2_EA:tanh_CA:linear_SEP:200_NO:L1_BS:16_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1',
                                        'LS:1024-2-2_ED:2_EA:tanh_CA:linear_SEP:200_NO:L1_BS:16_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1_SL:1']
        ...
        # the first model is the real one and the second is identical except with labels shuffled (for testing AUC statistical significance as in Pasolli)
        dataset_config['SingleDiseaseIBD'] = ['LS:1024-2_ED:2_EA:linear_CA:tanh_SEP:300_NO:L1_BS:16_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1',
                                              'LS:1024-2_ED:2_EA:linear_CA:tanh_SEP:300_NO:L1_BS:16_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1_SL:1'
        ]
        ...
        # the first model is the real one and the second is identical except with labels shuffled (for testing AUC statistical significance as in Pasolli)
        dataset_config['SingleDiseaseT2D'] = ['LS:1024-2_ED:2_EA:linear_CA:tanh_SEP:300_NO:L1_BS:32_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1',
                                              'LS:1024-2_ED:2_EA:linear_CA:tanh_SEP:300_NO:L1_BS:32_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1_SL:1']
        ...
        dataset_config['SingleDiseaseRA'] = ['LS:1024-2_ED:2_EA:linear_CA:linear_SEP:100_NO:L1_BS:32_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1',
                                             'LS:1024-2_ED:2_EA:linear_CA:linear_SEP:100_NO:L1_BS:32_BN:0_DO:0_AR:0_BE:tensorflow_V:2_UK:5__NR:1_SL:1'
        ]
