...
#just changed plot_ae_fold to True 080918
plot_ae_fold = False
...
def plot_tSNE(codes, name = 'tSNE', desc = '', config=None):
    ...
    plt.figure()
    n = 1
    ind = np.empty(shape=[0, n])
    for i in range(len(codes)):
        ind = np.append(ind, class_to_ind[info[i][label_ind]])
    y_test_cat = np_utils.to_categorical(ind, num_classes = 2)
...
#    for i in range(len(codes)):
#        ind = class_to_ind[info[i][label_ind]]
#        h = pylab.scatter([X_tsne[i][0]], [X_tsne[i][1]], color=colors[ind], alpha = 0.7)
#        handles[info[i][label_ind]] = h
#    keys = [ k for k in handles.keys() ]
#    keys.sort()
#    pylab.legend([handles[k] for k in keys], keys, prop={'size': plot_text_size})
    add_figtexts_and_save(fig, name, desc, config=config)
