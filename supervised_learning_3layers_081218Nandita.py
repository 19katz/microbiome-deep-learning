...
def run_model(data_set, kmer_size, norm_input, encoding_dim_1, encoding_dim_2,encoding_dim_3, encoded_activation, input_dropout_pct, dropout_pct, num_epochs, batch_size, n_splits, n_repeats, compute_informative_features, plot_iteration, graph_dir, outFile):
    
    # format strings for outputting the paramters associated with this run:
    summary_string, plotting_string= stats_utils.format_input_parameters_printing_3layers(data_set, kmer_size, norm_input, encoding_dim_1, encoding_dim_2,encoding_dim_3, encoded_activation,input_dropout_pct,dropout_pct,num_epochs,batch_size,n_splits,n_repeats,compute_informative_features,plot_iteration)
    
    outFile_header='data_set\tkmer_size\tnorm_input\tencoding_dim_1\tencoding_dim_2\tencoding_dim_3\tencoded_activation\tinput_dropout_pct\tdropout_pct\tnum_epochs\tbatch_size\tn_splits\tn_repeats\t'
    ...
    for n_repeat in range(0,len(rskf[0])):
        ...
        model=deep_learning_models.create_supervised_model_3layers(input_dim, encoding_dim_1, encoding_dim_2, encoding_dim_3, encoded_activation,input_dropout_pct, dropout_pct)
...
def parse_config_and_run(config_dict, outFile):
    ...
    encoding_dims_3=config_dict['encoding_dim_3']
