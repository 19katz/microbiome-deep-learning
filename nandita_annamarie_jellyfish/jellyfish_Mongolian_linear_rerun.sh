dataset=Mongolian
dir=/pollard/home/ngarud/shattuck/metagenomic_fastq_files/Mongolian/fastq_files_no_adapter
...
for kmer_size in 5 6 8 10; do
    ...
    jellyfish count <(zcat ${dir}/${file}-trimmed-pair1.fastq.gz) <(zcat ${dir}/${file}-trimmed-pair2.fastq.gz) /dev/fd/0 -m ${kmer_size} -s 100M -t 5 -C -F 2 -o ~/deep_learning_microbiome/data/${kmer_size}mers_jf/${dataset}/${file}_${kmer_size}mer.jf 
    ...
    # rerun jellyfish
    jellyfish count <(zcat ${dir}/${file}-trimmed-pair1.fastq.gz) <(zcat ${dir}/${file}-trimmed-pair1.fastq.gz) /dev/fd/0 -m $kmer_size -s 100M -t 5 -C -F 2 -o ~/deep_learning_microbiome/data/${kmer_size}mers_jf/${dataset}/${file}_${kmer_size}mer.jf
    ...
    rm ~/deep_learning_microbiome/data/${kmer_size}mers_jf/${dataset}/${file}_tmpOut.txt

    fi

done < ~/deep_learning_microbiome/analysis/${dataset}_${kmer_size}_rerun.txt
