...
def write_command(batch, kmer_size):
    ...
    bash_script=os.path.expanduser('~/deep_learning_microbiome/tmp_intermediate_files/') + 'MetaHIT_batch_' + str(batch) + '_bash_script.sh'
...
counter =0
batch = 1
list=os.path.expanduser('~/deep_learning_microbiome/tmp_intermediate_files/') + 'MetaHIT_batch_' + str(batch) + '.txt'  
outFile=open(list,'w')


command_file_inFN=os.path.expanduser('~/deep_learning_microbiome/tmp_intermediate_files/all_commands_MetaHIT_%smers.sh') % kmer_size
